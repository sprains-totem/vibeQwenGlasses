...
class GlassesConnectionService : Service() {
    ...
    private fun handleGcspControl(bytes: ByteArray) {
        com.vibeqwen.glasses.util.LogCollector.h("←发送 GCSP 控制/响应帧 (" + bytes.size + "B): " + bytes.take(16).joinToString("") { "%02X".format(it) })
    ...
    private fun startRecording(auto: Boolean = false) {
        ...
        // 下发官方触控长录音激活序列（Packet 16214~16222 官方 UI 点击完全对齐：Flag 0x00, NS 0x00, Cmd 0x01）
        ...
        // 官方 UI 现场触控录音标准四部曲（严禁 dialogId，严格携带 reason: "touch" 与 bizType: "live"）
        ...
        val j3 = """{"data":{"reason":"touch"},"pageType":"SCHEME_AIRECORD_START","sessionId":"$sessionIdStr","traceId":"$traceId","uri":"airecord://start"}"""
        ...
        scope.launch(Dispatchers.IO) {
            ...
            com.vibeqwen.glasses.util.LogCollector.r("←下发录音指令 1: AudioRecording (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j1.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
            delay(40)

            com.vibeqwen.glasses.util.LogCollector.r("←下发录音指令 2: scene=AudioRecording (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j2.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
            delay(40)

            com.vibeqwen.glasses.util.LogCollector.r("←下发录音指令 3: SCHEME_AIRECORD_START (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j3.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
            delay(30)

            com.vibeqwen.glasses.util.LogCollector.r("←下发录音指令 4: type=4 开启音频硬件推流 (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j4.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
        }
    ...
    private fun stopRecording() {
        ...
        // 官方抓包 Packet 16248~16255 严格对齐的完整停止与硬件通道注销序列 (Flag 0x00, NS 0x00, Cmd 0x01)
        ...
        scope.launch(Dispatchers.IO) {
            com.vibeqwen.glasses.util.LogCollector.r("←下发停止指令 1: PART (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j1.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
            delay(30)

            com.vibeqwen.glasses.util.LogCollector.r("←下发停止指令 2: AudioRecording (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j2.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
            delay(30)

            com.vibeqwen.glasses.util.LogCollector.r("←下发停止指令 3: 关闭硬件推流引擎 type=4 arg2=1 (ns=0x00, cmd=0x01, flag=0x00)")
            transport?.write(com.vibeqwen.glasses.protocol.QwenFramer.wrap(
                j3.toByteArray(Charsets.UTF_8), flag = 0x00, nameSpace = 0x00, cmdId = 0x01
            ))
        }
