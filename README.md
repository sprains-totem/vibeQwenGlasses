...
- **真机最新安装**：`versionCode=293`, `versionName=1.0.0-293-36256b2`
- **实测性能与声学反馈表现**：
  - 极速握手耗时从 8.8s 暴降至 0.95s（提速 900%）；
  - 启动录音精准触发眼镜硬件原生“噔噔”提示音（消除 `dialogId` 带来的 AI 对话第二轮监听声）；
  - 连续稳定采集 16kHz 16-bit 单声道 PCM 裸流（实测 5 秒 1307 帧无丢包），动态波形实时绘制；
  - 停止录音显式下发 `type:4, arg2:1` 释放 BES2800 硬件推流通道，触发眼镜原生结束提示音，无任何幽灵自启死循环。
